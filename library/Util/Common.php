...
class Util_Common
{
    ...
    public static function getMongoQB ($sType = 'bll')
    {
        if (! isset(self::$_aInstance['mongodb'][$sType])) {
            $aConf = self::getConf($sType, 'mongodb');
            if (empty($aConf)) {
    ...
    {
        if (! isset(self::$_aInstance['mysqldb'][$sDbName])) {
            self::$_aInstance['mysqldb'][$sDbName] = new Db_MySQL($sDbName);
        }
        return self::$_aInstance['mysqldb'][$sDbName];
    }
