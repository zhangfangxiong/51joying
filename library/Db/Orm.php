...
class Db_Orm
{
     ...
     * @var unknown
     */
    protected static $_aADUSQL = array();
     ...
     * 取得增删改SQL
     */
    public static function getADUSQL ()
    {
        return self::$_aADUSQL;
    }
...
//             }
    ...
    protected function _getMasterDB ()
    {
        if (null == $this->_oMasterDB) {
            ...
            $this->_oMasterDB = Util_Common::getPdoDb($this->_sDbName, 'master');
    ...
    private function _bindParams ($aParam)
    {
        if (! empty($aParam)) {      
            foreach ($aParam as $sField => $mValue) {
                if (is_array($mValue) && isset($mValue['FIELD'])) {
                    ...
                    if (isset($where_keys[$key])) {
                        foreach ($where_keys[$key] as $k => $v) {
                            switch ($tmp_where[$v][1]) {
                                case 'SET':
                                    $where[] = 'FIND_IN_SET("' . $tmp_where[$v][0] . '",`' . $key . '`)';
                                    break;
                                case 'IN':
    ...
    {
        $opts = $this->fixOpts($opts);
        $aRow = $this->fetchRow($opts, $bStrictMaster);
     ...
     * 补充查询条件
     */
    private function fixOpts($opts)
    {
        if (empty($opts)) {
            return $opts;
        }
        
        if (isset($opts['where']) || isset($opts['limit']) || isset($opts['order']) || isset($opts['group'])) {
            return $opts;
        }
        
        return array('where' => $opts);
    }
