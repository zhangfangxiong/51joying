 ...
 */
class Tijian_Controller_Tpa_Admin_Menu extends Tijian_Controller_Tpa_Admin_Base
{
    ...
    public function delAction ()
    {
        ...
        $iRet = Tijian_Model_Tpa_Menu::delData($iMenuID);
        if ($iRet == 1) {
    ...
    {
        $aTree = Tijian_Model_Tpa_Menu::getTree();
    ...
    public function editAction ()
    {
        if ($this->isPost()) {            
            ...
            $aOldMenu = Tijian_Model_Tpa_Menu::getDetail($aMenu['iMenuID']);
            if (empty($aOldMenu)) {
                return $this->showMsg('菜单不存在！', false);
            }
            // 更新排序，加在最后面
            if ($aOldMenu['iParentID'] != $aMenu['iParentID']) {
                $aMenu['iOrder'] = Tijian_Model_Tpa_Menu::getNextOrder($aMenu['iParentID']);
            }
            if (1 == Tijian_Model_Tpa_Menu::updData($aMenu)) {
            ...
            $iMenuID = intval($this->getParam('id'));
            $aMenu = Tijian_Model_Tpa_Menu::getDetail($iMenuID);
            $aTree = Tijian_Model_Tpa_Menu::getMenus();
            $this->assign('aTree', $aTree);
    ...
    public function moveAction ()
    {
        ...
        $aMenu = Tijian_Model_Tpa_Menu::getDetail($iMenuID);
        $iCnt = Tijian_Model_Tpa_Menu::changeOrder($aMenu, $iDirect);
    ...
    public function addAction ()
    {
        if ($this->isPost()) {
            ...
            $aMenu['iOrder'] = Tijian_Model_Tpa_Menu::getNextOrder($aMenu['iParentID']);
            if (Tijian_Model_Tpa_Menu::addData($aMenu) > 0) {
