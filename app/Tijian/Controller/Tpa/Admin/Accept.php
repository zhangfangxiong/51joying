 ...
 * Time: 9:50
 */
class Tijian_Controller_Tpa_Admin_Accept extends Tijian_Controller_Tpa_Admin_Base
{
    //受理雇员列表
    public function employeeAction()
    {
        ...
        $where['iStatus'] = Tijian_Model_Tpa_Employee::STATUS_VALID;
        ...
        $aData = Tijian_Model_Tpa_Employee::getList($where, $iPage, 'iUpdateTime DESC');
        if (!empty($aData['aList'])) {
            foreach ($aData['aList'] as $key => &$value) {
                $aCompany = Tijian_Model_Tpa_Company::getDetail($value['iCompanyID']);
                ...
                if (!empty($value['iParentID'])) {
                    $aParent = Tijian_Model_Tpa_Employee::getDetail($value['iParentID']);
    ...
    public function caseAction()
    {
        ...
        $aData = Tijian_Model_Tpa_Case::getList($where, $iPage, 'iUpdateTime DESC');
        if (!empty($aData['aList'])) {
            $aPriority = Yaf_G::getConf('aPriority');
            foreach ($aData['aList'] as $key => &$value) {
                $aUser = Tijian_Model_Tpa_Admin::getDetail($value['iCreateUserID']);
        ...
        $aUser = Tijian_Model_Tpa_Admin::getAll($aUserParam);
        //只展示该权限的用户
        //todo
        $this->assign('iType', 2);
        $this->assign('aData', $aData);
    ...
    public function errorAction()
    {
        ...
        $aData = Tijian_Model_Tpa_Error::getList($where, $iPage, 'iUpdateTime DESC');
        $aErrorType = Yaf_G::getConf('aErrorType');
        $aErrorStatus = Yaf_G::getConf('aErrorStatus');
        if (!empty($aData['aList'])) {
            foreach ($aData['aList'] as $key => &$value) {
                $aAdmin = Tijian_Model_Tpa_Admin::getDetail($value['iUpdateUserID']);
    ...
    public function addErrorAction()
    {
        ...
        if (empty($aParams['iID'])) {
            ...
            $aEmployee = Tijian_Model_Tpa_Employee::getDetail($aParams['iID']);
        ...
        if (Tijian_Model_Tpa_Error::addData($aParams)) {
    ...
    public function addCaseAction()
    {
        ...
        if (empty($iID) && empty($iErrorid)) {
            ...
            if (!empty($iID)) {
                $aEmployee = Tijian_Model_Tpa_Employee::getDetail($iID);
                if (empty($aEmployee)) {
                    return $this->showMsg('雇员不存在', false);
                }
            } elseif (!empty($iErrorid)) {
                $aError = Tijian_Model_Tpa_Error::getDetail($iErrorid);
                if (empty($aError)) {
                    return $this->showMsg('问题件不存在', false);
                }
                $aEmployee = Tijian_Model_Tpa_Employee::getEmployeeByIDAndName($aError['sIdentityCard'],$aError['sRealName']);
                if (empty($aEmployee)) {
        ...
        if ($this->isPost()) {
            $aParam = $this->getParams();
            $aParam['sCode'] = Tijian_Model_Tpa_Case::initCode();
        ...
        $aMedicalInsurancePrivinceID = Tijian_Model_Tpa_Region::getDetail($aEmployee['iMedicalInsurancePrivinceID']);
        $aMedicalInsuranceCityID = Tijian_Model_Tpa_Region::getDetail($aEmployee['iMedicalInsuranceCityID']);
        $aJobPrivinceID = Tijian_Model_Tpa_Region::getDetail($aEmployee['iJobPrivinceID']);
        $aJobCityID = Tijian_Model_Tpa_Region::getDetail($aEmployee['iJobCityID']);
        $aCompany = Tijian_Model_Tpa_Company::getDetail($aEmployee['iCompanyID']);
        ...
        $aClaimsplan = Tijian_Model_Tpa_Claimsplan::getAll($aClaimsplanParam);
        ...
        $aCase = Tijian_Model_Tpa_Error::getAll($aCaseParam);
        if (!empty($aCase)) {
    ...
    public function viewCaseAction()
    {
        ...
        $aThisCase = Tijian_Model_Tpa_Case::getDetail($iID);
        if (empty($aThisCase)) {
            return $this->showMsg('案件不存在', false);
        }
        $aEmployee = Tijian_Model_Tpa_Employee::getDetail($aThisCase['sEmployeeID']);
        if (empty($aEmployee)) {
            return $this->showMsg('雇员不存在', false);
        }
    ...
    public function printAction()
    {
        ...
        //todo
        //批量标记为已打印
        $iResult = Tijian_Model_Tpa_Case::printCase($iIDs, $this->aCurrUser['iUserID']);
        if ($iResult) {
