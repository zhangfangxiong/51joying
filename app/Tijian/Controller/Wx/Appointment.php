 ...
 * Time: 下午2:32
 */
class Tijian_Controller_Wx_Appointment extends Tijian_Controller_Wx_Base
{
    /**
     * 执行动作之前
     */
    ...
    public function refundAction()
    {
        ...
        $aProduct = Tijian_Model_UserProductBase::getUserProductBase($aCardProduct['iProductID'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);
        if (empty($aProduct)) {
        ...
        if (empty($aCardProduct['iRefunding'])) {//未退款
            ...
            $aCustomer = Tijian_Model_Customer::getDetail($this->iCurrUserID);
            if (empty($aCustomer)) {
            ...
            if($iRefundID = Tijian_Model_Refund::addData($aRefundParam)) {
            ...
            $aRefundParam['iStatus'] = Tijian_Model_Refund::REFUND_CANCEL;
            Tijian_Model_OrderCardProduct::begin();
    ...
    public function cardListAction()
    {
        ...
        $aCard = Tijian_Model_OrderCard::getUserCardList($this->iCurrUserID, $aPage);
        if (!empty($aCard['aList'])) {
            ...
            foreach ($aCard['aList'] as $key => &$value) {
                ...
                $aCardProduct = Tijian_Model_OrderCardProduct::getAll($aCardProductParam);
                ...
                if (!empty($aCardProduct)) {
                    foreach ($aCardProduct as $k => $v) {
                        $iChannelType = ($value['iOrderType'] == Tijian_Model_OrderCard::ORDERTYPE_PRODUCT || $value['iOrderType'] == Tijian_Model_OrderCard::ORDERTYPE_PRODUCT_PLAN) ? 1 : 2;

                        $aProduct = Tijian_Model_UserProductBase::getUserProductBase($v['iProductID'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);
                        ...
                        Tijian_Model_Refund::ifCanRefund($value,$v);
                        //print_r($v);die;
    ...
    public function addCardAction()
    {
        ...
        if ($aCard['iSex'] != $this->aUser['iSex']) {
            ...
            if (!empty($aCardProductID)) {
                ...
                foreach ($aCardProductID as $key => $value) {
                    $aProduct = Tijian_Model_UserProductBase::getUserProductBase($value, $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);
                    if (!empty($aProduct)) {
        ...
        if (Tijian_Model_OrderCard::updData($aAppointmentParam)) {
    ...
    public function mapAction()
    {
        ...
        $aStore = Tijian_Model_Store::getDetail($iStorID);
        if (empty($aStore)) {
    ...
    public function reserveStoreAction()
    {
        ...
        if (!empty($aParam['upid'])) {
            $aUpProduct = Tijian_Model_UserProductBase::getUserProductBase($aParam['upid'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);//升级产品
            if (empty($aUpProduct)) {
    ...
    public function reserveCommitAction()
    {
        ...
        $aProductStore = Tijian_Model_UserProductStore::getUserHasStore($iProductID, $aCard['iCompanyID'], $aParam['sid'], $iChannelType, $this->aUser['iChannel'], true);
        ...
        if ($this->isPost()) {
            ...
            $aCardProductParam['iPlat'] = Tijian_Model_OrderCardProduct::RESERVEPLAT_WX;
            Tijian_Model_OrderCardProduct::begin();
            if (Tijian_Model_OrderCardProduct::updData($aCardProductParam)) {
    ...
    public function reservecancelAction()
    {
        ...
        Tijian_Model_OrderCardProduct::begin();
        if (Tijian_Model_OrderCardProduct::updData($aProductParam)) {
            if ($aCard['iUseType'] == Tijian_Model_OrderCard::USETYPE_OR) {
    ...
    public function reserveDetailAction()
    {
        ...
        $aProductStore = Tijian_Model_UserProductStore::getUserHasStore($aParam['pid'], $aCard['iCompanyID'], $aCardProduct['iStoreID'], $iChannelType, $this->aUser['iChannel'], true);
        if (empty($aProductStore)) {
            return $this->show404('该门店不存在该产品', false);
        }
    ...
    public function upgradeDetailAction()
    {
        ...
        $aProduct = Tijian_Model_UserProductBase::getUserProductBase($aCardProduct['iProductID'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);//原产品
        if (empty($aProduct)) {
        ...
        //获取门店数目
        $aProductStore = Tijian_Model_UserProductStore::getUserProductStore($aUpgrade['iProductID'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);
        $aTmp = [];
        ...
        //获取该产品包含的单项
        $aItem = Tijian_Model_ProductItem::getProductItems($aUpgrade['iProductID'], Tijian_Model_ProductItem::EXPANDPRODUCT, null, true);
        if (!empty($aItem)) {
            $sItem = implode(',', $aItem);
        ...
        //获取原产品包含的单项
        $aItem1 = Tijian_Model_ProductItem::getProductItems($aProduct['iProductID'], Tijian_Model_ProductItem::EXPANDPRODUCT, null, true);
        //获取两个产品的单项合集
    ...
    public function detailAction()
    {
        ...
        //获取门店数目
        $aProductStore = Tijian_Model_UserProductStore::getUserProductStore($aProduct['iProductID'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);
        $aTmp = [];
        foreach ($aProductStore as $key => $value) {
            $aTmp[$value['iAutoID']] = $value['iStoreID'];
        }
        ...
        //获取该产品包含的单项
        $aItem = Tijian_Model_ProductItem::getProductItems($aProduct['iProductID'], Tijian_Model_ProductItem::EXPANDPRODUCT, null, true);
        if (!empty($aItem)) {
            $sItem = implode(',', $aItem);
            $aItemCat = Tijian_Model_Item::setGroupByCategory($sItem, true);
        } else {
        ...
        //获取所有升级产品
        $aUpgrade = Tijian_Model_UserProductUpgrade::getUserProductUpgrade($aProduct['iProductID'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);
        $iTime = time();
        //组装升级产品所需数据
        if (!empty($aUpgrade)) {
            $aCardProducts = Tijian_Model_OrderCardProduct::getProductByCardIDs($aParam['id']);//获取该卡中所有产品id
            foreach ($aUpgrade as $key => $value) {
                $aUpgradeDetail = Tijian_Model_UserProductBase::getUserProductBase($value['iUpgradeID'], $aCard['iCompanyID'], $iChannelType, $this->aUser['iChannel']);
