...
class Tijian_Model_Refund extends Tijian_Model_Base
{
    ...
    public static function ifCanRefund($aCard, &$aCardProduct)
    {
        ...
        if (in_array($aCardProduct['iBookStatus'], [0, 3, 6])) {//未预约或已退订或预约失败状态可退款
            if (($aCard['iOrderType'] == Tijian_Model_OrderCard::ORDERTYPE_PRODUCT || $aCard['iOrderType'] == Tijian_Model_OrderCard::ORDERTYPE_PRODUCT_PLAN) && $aCardProduct['iPayStatus'] == Tijian_Model_OrderCard::PAYSTATUS_1 && $aCard['iPayType'] == Tijian_Model_OrderCard::PAYTYPE_PERSON) {
        ...
        if (!empty($aCardProduct['iLastProductID'])) {//升级过的
            if (!empty($aCardProduct['iLastOPID'])) {
                if (($aCard['iOrderType'] == Tijian_Model_OrderCard::ORDERTYPE_PRODUCT || $aCard['iOrderType'] == Tijian_Model_OrderCard::ORDERTYPE_PRODUCT_PLAN) && $aCardProduct['iPayStatus'] == Tijian_Model_OrderCard::PAYSTATUS_1 && $aCard['iPayType'] == Tijian_Model_OrderCard::PAYTYPE_PERSON) {
                    //1，体检产品或体检计划的个人支付，且已支付过
                    $aOrderProduct = Tijian_Model_OrderProduct::getDetail($aCardProduct['iLastOPID']);
                    ...
                    $aOrderInfo = Tijian_Model_OrderInfo::getDetail($aCardProduct['iLastOrderID']);
                    $aCardProduct['sOrderCode'] = $aOrderInfo['sOrderCode'];
                    $aCardProduct['sPayOrderID'] = $aOrderInfo['sPayOrderID'];
                }
            }
            $aOrderUpProduct = Tijian_Model_OrderProduct::getDetail($aCardProduct['iOPID']);
            ...
            $aOrderInfo = Tijian_Model_OrderInfo::getDetail($aCardProduct['iOrderID']);
