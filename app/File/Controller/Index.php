 ...
 * Time: 下午2:32
 */
class File_Controller_Index extends Yaf_Controller
{
    ...
    public function uploadAction ()
    {
        ...
        $needCheckQRCode = false;
        $paramCheck = $this->getParam("nc");
        if ($paramCheck == "qrcode") {
        ...
        if (true === $mResult) {
            ...
            if ($needCheckQRCode) {
                foreach ($aFiles as $aFile) {
                    $canUpload = $this->_isQRCode($aFile['tmp_name']);
                }
            }
            ...
            if ($canUpload) {
                ...
                foreach ($aFiles as $aFile) {
                    ...
                    if ($iError == 0) {
                        ...
                        $mResult['sName'] = $aFile['name'];
                        $aUpdFiles[$aFile['key']] = $mResult;
                        // $aUpdFiles[$aFile['key']] = $aFile['tmp_name'];
                    } else {
                        $aUpdFiles[$aFile['key']] = array(
                            'iError' => $iError,
                            'sMsg' => $mResult
                        );
                    }
            ...
            $iRetError = 1;
            $mResult = array(
                'sMsg' => 'The upload domain is forbidden.'
            );
        }
        
        $this->getResponse()->setHeader('Content-Type', 'application/json');
        $aRet = array_merge(array(
            'iError' => $iRetError
        ), $mResult);
    ...
    private function _isQRCode ($img_file)
    {
        require_once LIB_PATH . '/QRCode/QrReader.php';
        $result = true;
        try {
    ...
    public function banneruploadAction ()
    {
        ...
        if (true === $mResult) {
            ...
            // 批量上传图片 @todo 待优化
            foreach ($aFiles as $aFile) {
                list ($iError, $mResult) = $oStorage->saveFile($aFile['name'], $aFile['tmp_name'], $aFile['error'], $aFile['size'], $sIP, $sDomain, File_Model_FileMeta::BID_BANNER);
    ...
    public function viewAction ()
    {
        ...
        if (isset($params['key'])) {
            ...
            $iBid = File_Model_FileMeta::BID_DEFAULT;
            if (isset($params['biz']) && 'banner' == $params['biz']) {
                $iBid = File_Model_FileMeta::BID_BANNER;
            }
            ...
            if (in_array($mRet, [
                File_Storage::FILE_NOT_EXISTS,
                File_Storage::FILE_EXT_INVAILD,
                File_Storage::FILE_MIMETYPE_NOT_ALLOWED
            ])) {
     ...
     * @param
     *            $p_bCrop
     * @return string ETag memcache key
     */
