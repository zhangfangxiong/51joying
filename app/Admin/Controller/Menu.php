...
class Admin_Controller_Menu extends Admin_Controller_Base
{
    ...
    public function delAction ()
    {
        ...
        $iRet = Admin_Model_Menu::delData($iMenuID);
        if ($iRet == 1) {
    ...
    {
        $aTree = Admin_Model_Menu::getTree();
    ...
    public function editAction ()
    {
        if ($this->isPost()) {
            ...
            $aOldMenu = Admin_Model_Menu::getDetail($aMenu['iMenuID']);
            if (empty($aOldMenu)) {
                return $this->showMsg('菜单不存在！', false);
            }
            // 更新排序，加在最后面
            if ($aOldMenu['iParentID'] != $aMenu['iParentID']) {
                $aMenu['iOrder'] = Admin_Model_Menu::getNextOrder($aMenu['iParentID']);
            }
            if (1 == Admin_Model_Menu::updData($aMenu)) {
            ...
            $iMenuID = intval($this->getParam('id'));
            $aMenu = Admin_Model_Menu::getDetail($iMenuID);
            $aTree = Admin_Model_Menu::getMenus();
            $this->assign('aTree', $aTree);
    ...
    public function moveAction ()
    {
        ...
        $aMenu = Admin_Model_Menu::getDetail($iMenuID);
        $iCnt = Admin_Model_Menu::changeOrder($aMenu, $iDirect);
    ...
    public function addAction ()
    {
        if ($this->isPost()) {
            ...
            $aMenu['iOrder'] = Admin_Model_Menu::getNextOrder($aMenu['iParentID']);
            if (Admin_Model_Menu::addData($aMenu) > 0) {
