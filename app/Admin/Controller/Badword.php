...
class Admin_Controller_Badword extends Admin_Controller_Base
{
    ...
    public function delAction ()
    {
        ...
        $iRet = Admin_Model_Badword::delData($iBadwordID);
        if ($iRet == 1) {
    ...
    public function listAction ()
    {
        ...
        $aList = Admin_Model_Badword::getList($aWhere, $iPage);
    ...
    public function editAction ()
    {
        if ($this->_request->isPost()) {            
            ...
            $aOldBadword = Admin_Model_Badword::getDetail($aBadword['iAutoID']);
            if (empty($aOldBadword)) {
                return $this->showMsg('敏感词不存在！', false);
            }
            if ($aOldBadword['sWord'] != $aBadword['sWord']) {
                if (Admin_Model_Badword::getBadwordByWord($aBadword['sWord'])) {
                    return $this->showMsg('敏感词已经存在！', false);
                }
            }
            if (1 == Admin_Model_Badword::updData($aBadword)) {
            ...
            $iBadwordID = intval($this->getParam('id'));
            $aBadword = Admin_Model_Badword::getDetail($iBadwordID);
    ...
    public function addAction ()
    {
        if ($this->_request->isPost()) {
            ...
            if (Admin_Model_Badword::addData($aBadword) > 0) {
