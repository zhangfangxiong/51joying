...
class Admin_Controller_Article extends Admin_Controller_Base
{
    ...
    public function __call ($sMethod, $aArg)
    {
        ...
        $aClass = Admin_Model_Article::getClass($sClass);
        if (empty($aClass)) {
            parent::__call($sMethod, $aArg);
            return false;
        }
        $this->listAction($sClass);
        $this->setViewScript('/Article/list.phtml');
    }
...
//        if ($aClass['sCityID'] > 0) {
//            $aParam['sCityID'] = $this->iCurrCityID;
//        }
        ...
        $aList = Admin_Model_Article::getPageList($aParam, $iPage, $sOrder);
        $this->assign('aParam', $aParam);
        $this->assign('aList', $aList);
        $this->assign('aClass', $aClass);
        $this->assign('aCategory', Admin_Model_Article::getCategorys($sClass));
    ...
    public function offAction()
    {
        ...
        foreach ($iArticleID as $key => $value) {
            $aArticle[$key] = Admin_Model_Article::getDetail($value);
            $aRow = array(
                'iArticleID' => $value,
                'iPublishStatus' => 0
            );
            $iRet = Admin_Model_Article::updData($aRow);
            if ($iRet != 1) {
                $fail_article[] = $value;
            } else {
                $secc_article[] = $value;
    ...
    public function delAction()
    {
        ...
        $fail_article = array();
        foreach ($iArticleID as $key => $value) {
            $iRet = Admin_Model_Article::delData($value);
            if ($iRet != 1) {
                $fail_article[] = $value;
            }
        }
    ...
    public function editAction()
    {
        if ($this->isPost()) {
            ...
            $aClass = Admin_Model_Article::getClass($aArticle['sClass']);
            //修改需要加上当前修改人ID
            $aCurrUserInfo = $this->aCurrUser;
            $aArticle['iUpdateUserID'] = $aCurrUserInfo['iUserID'];
            if (1 == Admin_Model_Article::updData($aArticle)) {
            ...
            $aArticle = Admin_Model_Article::getDetail($iArticleID);
            if ($aArticle['sTag']) {
                $aArticle['aTag'] = explode(',', $aArticle['sTag']);
            }

            $this->assign('aArticle', $aArticle);
            $this->assign('aCategory', Admin_Model_Article::getCategorys($aArticle['sClass']));
            $this->assign('aTag', Admin_Model_Article::getTags($aArticle['sClass']));
            $this->assign('aAuthor', Admin_Model_Article::getAuthors($aArticle['sClass']));
            ...
            $this->assign('aClass', Admin_Model_Article::getClass($aArticle['sClass']));
        }
    }
    ...
    public function addAction()
    {
        if ($this->isPost()) {
            ...
            $iArticleID = Admin_Model_Article::addData($aArticle);
            if ($iArticleID > 0) {
            ...
            $this->assign('aCategory', Admin_Model_Article::getCategorys($sClass));
            $this->assign('aTag', Admin_Model_Article::getTags($sClass));
            $this->assign('aAuthor', Admin_Model_Article::getAuthors($sClass));
            $this->assign('sUploadUrl', Yaf_G::getConf('upload', 'url'));
            $this->assign('sFileBaseUrl', 'http://' . Yaf_G::getConf('file', 'domain'));
            $this->assign('aClass', Admin_Model_Article::getClass($sClass));
    ...
    public function _checkData($sType = 'add', $aParam = array())
    {
        ...
        $aClass = Admin_Model_Article::getClass($aRow['sClass']);
        ...
        if (! empty($aParam) || (isset($aRow['iOptype']) && $aRow['iOptype'] > 0)) {
            ...
            if ($aClass['sAuthor']) {
                ...
                if (!Admin_Model_Type::getTypeByName($sAuthorType, $aRow['sAuthorName'])) {
            ...
            if ($aClass['sTag'] && $aRow['sTag']) {
                ...
                foreach ($sTag as $key => $value) {
                    $aTag = Admin_Model_Type::getDetail($value);
    ...
    public function emailAction() {
        ...
        $aList = Admin_Model_ArticleEmail::getList([], $page, 'iAutoID DESC', 20);
        if ($aList['aList']) {
            foreach($aList['aList'] as $key => $value) {
                $article = Admin_Model_Article::getDetail($value['iArticleID']);
                $aList['aList'][$key]['article'] = $article;
            }
        }
        $this->assign('aList', $aList);
    }
    ...
    public function sendAction()
    {
        ...
        $data['iAutoID'] = $id;
        $data['iSend'] = 2;
        Admin_Model_ArticleEmail::updData($data);
        return $this->showMsg('数据更新成功', true);
    }
}