...
class Admin_Controller_Front extends Admin_Controller_Base
{
    ...
    public function __call ($sMethod, $aArg)
    {
        ...
        $aClass = Admin_Model_Front::getClass($sClass);
        if (empty($aClass)) {
            parent::__call($sMethod, $aArg);
            return false;
        }
        $this->listAction($sClass);
        $this->setViewScript('/Front/list.phtml');
    }
    ...
    public function delAction ()
    {
        ...
        $aFront = Admin_Model_Front::getDetail($iAutoID);
        $sClassName = Admin_Model_Front::getClass($aFront['sClass'], 'sTitle');
        $iRet = Admin_Model_Front::delData($iAutoID);
        if ($iRet == 1) {
    ...
    public function listAction ($sClass = '')
    {
        ...
        $aClass = Admin_Model_Front::getClass($sClass);
        $aList = Admin_Model_Front::getFronts($sClass);
    ...
    public function editAction ()
    {
        if ($this->isPost()) {
            ...
            $aClass = Admin_Model_Front::getClass($aFront['sClass']);
            $aFront['iAutoID'] = intval($this->getParam('iAutoID'));
            $aOldType = Admin_Model_Front::getDetail($aFront['iAutoID']);
            if (empty($aOldType)) {
                return $this->showMsg($aClass['sName'] . '不存在！', false);
            }
            if (1 == Admin_Model_Front::updData($aFront)) {
    ...
    public function addAction ()
    {
        if ($this->isPost()) {
            ...
            $aClass = Admin_Model_Front::getClass($aFront['sClass']);
            $aFront['iOrder'] = Admin_Model_Front::getNextOrder($aFront['sClass'], $aFront['iCityID']);
            if (Admin_Model_Front::addData($aFront) > 0) {
    ...
    public function moveAction ()
    {
        ...
        $aFront = Admin_Model_Front::getDetail($iAutoID);
        $iCnt = Admin_Model_Front::changeOrder($aFront, $iDirect);
