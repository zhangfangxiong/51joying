...
class Admin_Cmd_Order extends Admin_Cmd_Base
{
    ...
    public function getOrderAction()
    {
        ...
        if (empty($time)) {
            $time = Tpa_Model_Kv::getValue($key);
            if (empty($time)) {
                $time = $runtime;
            }
            Tpa_Model_Kv::setValue($key, $runtime);
        }
...
//            print_r($orderList);//exit;
            ...
            foreach($orderList['response']['trades'] as $key => $value) {
                ...
                if (count($arr) == 2) {
                    if ($arr['0'] == 'dashang') {
                        $row = Tpa_Model_JudgmentReward::getRow([
                                'where' => [
                                    'sOutID' => $sOutID
                                ]
                            ]);
                        ...
                        if ($row) {
                            ...
                            $updata['iPay'] = 2;
                            Tpa_Model_JudgmentReward::updData($updata);
                            Tpa_Model_Youzan::offGood($row['sGoodNum']);
                        } else {
                            continue;
                        }
                    } else if ($arr['0'] == 'zixunfei') {
                        $row = Tpa_Model_CustomerConsultation::getRow([
                                'where' => [
                                    'sOutID' => $sOutID
                                ]
                            ]);
