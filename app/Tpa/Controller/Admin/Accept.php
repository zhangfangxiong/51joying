 ...
 * Time: 9:50
 */
class Tpa_Controller_Admin_Accept extends Tpa_Controller_Admin_Base
{
    //受理雇员列表
    public function employeeAction()
    {
        ...
        $aData = Tpa_Model_Employee::getList($where, $iPage, 'iUpdateTime DESC');
        if (!empty($aData['aList'])) {
            foreach ($aData['aList'] as $key => &$value) {
                $aCompany = Tpa_Model_Company::getDetail($value['iCompanyID']);
                ...
                if (!empty($value['iParentID'])) {
                    $aParent = Tpa_Model_Employee::getDetail($value['iParentID']);
    ...
    public function caseAction()
    {
        ...
        $aData = Tpa_Model_Case::getList($where, $iPage, 'iUpdateTime DESC');
        if (!empty($aData['aList'])) {
            $aPriority = Yaf_G::getConf('aPriority');
            foreach ($aData['aList'] as $key => &$value) {
                $aUser = Tpa_Model_User::getDetail($value['iCreateUserID']);
        ...
        $aUser = Tpa_Model_User::getAll($aUserParam);
        //只展示该权限的用户
        //todo
        $this->assign('iType', 2);
        $this->assign('aData', $aData);
    ...
    public function errorAction()
    {
        ...
        $aData = Tpa_Model_Error::getList($where, $iPage, 'iUpdateTime DESC');
        $aErrorType = Yaf_G::getConf('aErrorType');
        $aErrorStatus = Yaf_G::getConf('aErrorStatus');
        if (!empty($aData['aList'])) {
            foreach ($aData['aList'] as $key => &$value) {
                $aAdmin = Tpa_Model_User::getDetail($value['iUpdateUserID']);
    ...
    public function addErrorAction()
    {
        ...
        if (empty($aParams['iID'])) {
            ...
            $aEmployee = Tpa_Model_Employee::getDetail($aParams['iID']);
        ...
        $aParams['sCode'] = Tpa_Model_Error::initCode();
        if (Tpa_Model_Error::addData($aParams)) {
    ...
    public function addCaseAction()
    {
        ...
        if (empty($iID) && empty($iErrorid)) {
            ...
            if (!empty($iID)) {
                $aEmployee = Tpa_Model_Employee::getDetail($iID);
                if (empty($aEmployee)) {
                    return $this->showMsg('雇员不存在', false);
                }
            } elseif (!empty($iErrorid)) {
                $aError = Tpa_Model_Error::getDetail($iErrorid);
                if (empty($aError)) {
                    return $this->showMsg('问题件不存在', false);
                }
                $aEmployee = Tpa_Model_Employee::getEmployeeByIDAndName($aError['sIdentityCard'],$aError['sRealName']);
                if (empty($aEmployee)) {
        ...
        if ($this->isPost()) {
            $aParam = $this->getParams();
            $aParam['sCode'] = Tpa_Model_Case::initCode();
            ...
            if (Tpa_Model_Case::addData($aParam)) {
        ...
        $aMedicalInsurancePrivinceID = Tpa_Model_Region::getDetail($aEmployee['iMedicalInsurancePrivinceID']);
        $aMedicalInsuranceCityID = Tpa_Model_Region::getDetail($aEmployee['iMedicalInsuranceCityID']);
        $aJobPrivinceID = Tpa_Model_Region::getDetail($aEmployee['iJobPrivinceID']);
        $aJobCityID = Tpa_Model_Region::getDetail($aEmployee['iJobCityID']);
        $aCompany = Tpa_Model_Company::getDetail($aEmployee['iCompanyID']);
        ...
        $aClaimsplan = Tpa_Model_Claimsplan::getAll($aClaimsplanParam);
        ...
        $aCase = Tpa_Model_Error::getAll($aCaseParam);
        if (!empty($aCase)) {
    ...
    public function viewCaseAction()
    {
        ...
        $aThisCase = Tpa_Model_Case::getDetail($iID);
        if (empty($aThisCase)) {
            return $this->showMsg('案件不存在', false);
        }
        $aEmployee = Tpa_Model_Employee::getDetail($aThisCase['sEmployeeID']);
        if (empty($aEmployee)) {
            return $this->showMsg('雇员不存在', false);
        }
    ...
    public function printAction()
    {
        ...
        //todo
        //批量标记为已打印
        $iResult = Tpa_Model_Case::printCase($iIDs, $this->aCurrUser['iUserID']);
        if ($iResult) {
