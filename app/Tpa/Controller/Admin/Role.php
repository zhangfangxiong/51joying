 ...
 * @Author xuchuyuan
 */
class Tpa_Controller_Admin_Role extends Tpa_Controller_Admin_Base
{
    ...
    public function listAction()
    {
        ...
        $aData = Tpa_Model_Role::getList($aWhere, $iPage, '', 20);
        $this->assign('sHrRoleName', Tpa_Model_Role::HRROLENAME);
        $this->assign('sSupplierRoleName', Tpa_Model_Role::SUPPLIERROLENAME);
        $this->assign('aData', $aData);
        $this->assign('aParam', $aParam);
        $this->assign('aType', Tpa_Model_User::$aType);
        $this->assign('aStatus', Tpa_Model_Role::$aStatus);
    }
    ...
    public function editAction()
    {
        ...
        if ($this->isPost()) {
            ...
            if (1 == Tpa_Model_Role::updData($aRole)) {
            ...
            $aRole = Tpa_Model_Role::getDetail($iRoleID);
            $aRole['aPermission'] = explode(',', $aRole['sPermission']);
            $aRole['aModule'] = explode(',', $aRole['sModule']);
            $this->assign('aRole', $aRole);
            $this->assign('aPermissionList', Tpa_Model_Permission::getAllPermissions());
            $this->assign('aMenuList', Tpa_Model_Menu::getMenus());
            $this->assign('aType', Tpa_Model_User::$aType);
    ...
    {
        $iType = $this->getParam('type');
        $iType = in_array($iType, array_keys(Tpa_Model_User::$aType)) ? $iType : 1;
        if ($this->isPost()) {
            ...
            if (Tpa_Model_Role::addData($aRole) > 0) {
    ...
    {
        $iRoleID = intval($this->getParam('id'));
        $iRet = Tpa_Model_Role::delData($iRoleID);
        if ($iRet == 1) {
    ...
    public function _checkData($sType = 'add')
    {
        ...
        if ($sType == 'add' && Tpa_Model_Role::getRoleByName($sRoleName, $iType)) {
