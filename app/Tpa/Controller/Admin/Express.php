 ...
 * Time: 9:50
 */
class Tpa_Controller_Admin_Express extends Tpa_Controller_Admin_Base
{
    //快递收入列表
    public function listAction()
    {
        ...
        if (in_array($iType, [1, 2])) {
            $where['iType'] = $iType;
            $where['iStatus'] = Tpa_Model_Express::NOPRINT;
        } else {
            ...
            $where['iStatus'] = Tpa_Model_Express::HASPRINT;

        }
        $aData = Tpa_Model_Express::getList($where, $iPage,'iUpdateTime DESC');
    ...
    public function addAction()
    {
        ...
        if (!empty($aParams['type']) && in_array($aParams['type'], [Tpa_Model_Express::TYPESELF, Tpa_Model_Express::TYPEEXPRESS])) {
            if ($aParams['type'] == Tpa_Model_Express::TYPESELF) {
                if (empty($aParams['sExpressCode'])) {
                    return $this->showMsg('快递单号不能为空！', false);
                }
                $aExpress = Tpa_Model_Express::getExpressByCode($aParams['sExpressCode']);
                if (!empty($aExpress)) {
                    return $this->showMsg('该快递单号已经录入，请检查是否单号输入有误！', false);
                }
            } elseif ($aParams['type'] == Tpa_Model_Express::TYPEEXPRESS) {
                $aParams['sExpressCode'] = Tpa_Model_Express::initExpressCode();
            } else {
            ...
            if (Tpa_Model_Express::addData($aParams)) {
    ...
    public function delAction()
    {
        ...
        $aParam['iStatus'] = 0;
        $aParam['iAutoID'] = $iID;
        if (Tpa_Model_Express::updData($aParam)) {
    ...
    public function printAction()
    {
        ...
        //todo
        //批量标记为已打印
        $iResult = Tpa_Model_Express::printExpress($iIDs,$this->aCurrUser['iUserID']);
        if ($iResult) {
