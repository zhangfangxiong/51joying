 ...
 * Time: 9:50
 */
class Tpa_Controller_Admin_Inquire extends Tpa_Controller_Admin_Base
{
    //雇员保单列表
    public function policyAction()
    {
        ...
        if (!empty($aParam['sCompanyName'])) {////匹配公司名称
            ...
            $company = Tpa_Model_Company::getRow(['where' => $cWhere]);
            if(!empty($company)) {
        ...
        $aData = Tpa_Model_Claimsplan::getList($where, $iPage);
    ...
    public function messagesAction()
    {
        ...
        $aData = Tpa_Model_Express::getList($where, $iPage);
        $this->assign('aData', $aData);
        $this->assign('aParam', $aParam);
    }
