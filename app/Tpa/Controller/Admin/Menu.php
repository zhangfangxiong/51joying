 ...
 */
class Tpa_Controller_Admin_Menu extends Tpa_Controller_Admin_Base
{
    ...
    public function delAction ()
    {
        ...
        $iRet = Tpa_Model_Menu::delData($iMenuID);
        if ($iRet == 1) {
    ...
    public function editAction ()
    {
        if ($this->isPost()) {            
            ...
            $aOldMenu = Tpa_Model_Menu::getDetail($aMenu['iMenuID']);
            if (empty($aOldMenu)) {
                return $this->showMsg('菜单不存在！', false);
            }
            // 更新排序，加在最后面
            if ($aOldMenu['iParentID'] != $aMenu['iParentID']) {
                $aMenu['iOrder'] = Tpa_Model_Menu::getNextOrder($aMenu['iParentID']);
            }
            if (1 == Tpa_Model_Menu::updData($aMenu)) {
            ...
            $iMenuID = intval($this->getParam('id'));
            $aMenu = Tpa_Model_Menu::getDetail($iMenuID);
            $aTree = Tpa_Model_Menu::getMenus();
            $this->assign('aTree', $aTree);
    ...
    public function moveAction ()
    {
        ...
        $aMenu = Tpa_Model_Menu::getDetail($iMenuID);
        $iCnt = Tpa_Model_Menu::changeOrder($aMenu, $iDirect);
    ...
    public function addAction ()
    {
        if ($this->isPost()) {
            ...
            $aMenu['iOrder'] = Tpa_Model_Menu::getNextOrder($aMenu['iParentID']);
            if (Tpa_Model_Menu::addData($aMenu) > 0) {
