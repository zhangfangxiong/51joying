 ...
 * Time: 9:50
 */
class Tpa_Controller_Admin_User extends Tpa_Controller_Admin_Base
{
    ...
    {
        parent::actionAfter();
        $this->assign('aType', Tpa_Model_User::$aType);
    }
    ...
    public function listAction()
    {
        ...
        $aWhere['iType'] = Tpa_Model_User::TYPE_ADMIN;
        $aData = Tpa_Model_User::getList($aWhere, $iPage);
        $this->assign('aData', $aData);
        $this->assign('aParam', $aParam);
        $this->assign('aStatus', Tpa_Model_User::$aStatus);
    ...
    public function adminListAction()
    {
        ...
        $this->assign('aParam', $aParam);
        $this->assign('aStatus', Tpa_Model_User::$aStatus);
        $this->assign('aRole', Model_Role::getPairRoles(Tpa_Model_User::TYPE_ADMIN, null));
    }
    ...
    public function hrListAction()
    {
        ...
        $aWhere['iType'] = Tpa_Model_User::TYPE_HR;
        $aData = Tpa_Model_User::getList($aWhere, $iPage);
        $this->assign('aData', $aData);
        $this->assign('aParam', $aParam);
        $this->assign('aStatus', Tpa_Model_User::$aStatus);
    ...
    public function userListAction()
    {
        ...
        if (!empty($aParam['sCompanyName'])) {
            $aCompany = Tpa_Model_User::getUserByRealName(trim($aParam['sCompanyName']), Tpa_Model_User::TYPE_HR);
            if (!empty($aCompany)) {
            ...
            foreach ($aData['aList'] as $key => $value) {
                ...
                if (!empty($aCompanys)) {
                    foreach ($aCompanys as $k => $v) {
                        $aCompany = Tpa_Model_User::getDetail($k);
    ...
    public function lockUserAction()
    {
        ...
        $aUser = Tpa_Model_User::getDetail($iUserID);
        if (empty($aUser)) {
            return $this->showMsg('该用户不存在！', false);
        }
        $aUser['iStatus'] = ($aUser['iStatus'] == Tpa_Model_User::STATUS_TYPE_LOCK) ? Tpa_Model_User::STATUS_TYPE_NORMAL : Tpa_Model_User::STATUS_TYPE_LOCK;
        $sType = ($aUser['iStatus'] == Tpa_Model_User::STATUS_TYPE_LOCK) ? '锁定' : '解锁';
        if (Tpa_Model_User::updData($aUser) > 0) {
    ...
    public function editAction()
    {
        if ($this->_request->isPost()) {
            ...
            $this->assign('aUser', $aUser);
            $this->assign('aSex', Tpa_Model_User::$aSex);
            $this->assign('aStatus', Tpa_Model_User::$aStatus);
            $this->assign('aDept', Model_Type::getOption('dept'));
    ...
    public function addAction()
    {
        if ($this->_request->isPost()) {
            ...
            $aUser['sUserName'] = Tpa_Model_User::initUserName(1);
            $aUser['iType'] = Tpa_Model_User::TYPE_ADMIN;
            $aUser['iStatus'] = Tpa_Model_User::STATUS_TYPE_NORMAL;
            $aUser['iIsCheck'] = Tpa_Model_User::ISCHECK;
            $aUser['sPassword'] = md5(Yaf_G::getConf('cryptkey', 'cookie') . $aUser['sUserName']);
            $aUser['iCreateUserID'] = $aUser['iLastUpdateUserID'] = $this->aCurrUser['iUserID'];
            if (Tpa_Model_User::addData($aUser) > 0) {
    ...
    public function _checkData($iType)
    {
        ...
        if ($iType == 2) {
            ...
            if (!Tpa_Model_User::getDetail($aParam['iUserID'])) {
                return $this->showMsg('用户不存在！', false);
            }
        }
        //验证邮箱
        if (!empty($aParam['sEmail'])) {
            $aUser = Tpa_Model_User::getUserByEmail($aParam['sEmail'], Tpa_Model_User::TYPE_ADMIN);
        ...
        if (!empty($aParam['sMobile'])) {
            $aUser = Tpa_Model_User::getUserByMobile($aParam['sMobile'], Tpa_Model_User::TYPE_ADMIN);
            if (!empty($aUser['iUserID']) && $aUser['iUserID'] != $aParam['iUserID']) {
    ...
    public function clientlistAction()
    {
        ...
        $aCustomerManager = Tpa_Model_User::getPairUser(Tpa_Model_User::TYPE_ADMIN, Tpa_Model_User::STATUS_TYPE_NORMAL);
        $this->assign('aData', $aData);
        $this->assign('aParam', $aParam);
        $this->assign('aIsCheck', Tpa_Model_User::$aIsCheck);
    ...
    public function clientcheckAction()
    {
        ...
        $aWhere['iType'] = Tpa_Model_User::TYPE_HR;
        $aWhere['iIsCheck IN'] = [Tpa_Model_User::NOCHECK, Tpa_Model_User::REFUSE];
    ...
    public function clientInfoAction()
    {
        ...
        $aIsCheck = Tpa_Model_User::$aIsCheck;
        $aParam['where']['iStatus'] = Tpa_Model_User::STATUS_TYPE_NORMAL;
        $aParam['where']['iType'] = Tpa_Model_User::TYPE_HR;
        $aCustomerManager = Tpa_Model_User::getPairUser(Tpa_Model_User::TYPE_ADMIN, Tpa_Model_User::STATUS_TYPE_NORMAL);
        $aUser['iLockUser'] = ($aUser['iStatus'] == Tpa_Model_User::STATUS_TYPE_LOCK) ? 1 : 0;
                    ...
                    if (!empty($aChannel)) {
                        ...
                        foreach ($aChannel as $k => $val) {
                            ...
                            if ($val['iViewRange'] == 0 || $val['iViewRange'] == 2) {//全部和不可见要统计渠道所有支持数目
                                ...
                                $aChannelUser = Tpa_Model_User::getCnt($aUserParam);
    ...
    public function clientplanAction()
    {
        ...
        $aUser = Tpa_Model_User::getDetail($userId);
    ...
    public function clientCheckDetailAction()
    {
        if ($this->_request->isPost()) {
            ...
            if (Tpa_Model_User::updData($aParam)) {
            ...
            $aIsCheck = Tpa_Model_User::$aIsCheck;
            $aParam['where']['iStatus'] = Tpa_Model_User::STATUS_TYPE_NORMAL;
            $aParam['where']['iType'] = Tpa_Model_User::TYPE_ADMIN;
            $aCustomerManager = Tpa_Model_User::getPairUser(Tpa_Model_User::TYPE_ADMIN, Tpa_Model_User::STATUS_TYPE_NORMAL);
            $aUser['iLockUser'] = ($aUser['iStatus'] == Tpa_Model_User::STATUS_TYPE_LOCK) ? 1 : 0;
            $this->assign('aIndustry', $aIndustry);
            $this->assign('aProperty', $aProperty);
    ...
    public function addClientAction()
    {
        if ($this->_request->isPost()) {
            ...
            $aUser['iType'] = Tpa_Model_User::TYPE_HR;
            $aUser['iIsCheck'] = Tpa_Model_User::NOCHECK;
            $aUser['sPassword'] = md5(Yaf_G::getConf('cryptkey', 'cookie') . $aUser['sUserName']);
            $aUser['iCreateUserID'] = $aUser['iLastUpdateUserID'] = $this->aCurrUser['iUserID'];
            ...
            if ($aUser['iLockUser'] == 1) {
                $aUser['iStatus'] = Tpa_Model_User::STATUS_TYPE_LOCK;
            } else {
                $aUser['iStatus'] = Tpa_Model_User::STATUS_TYPE_NORMAL;
            }
            if (Tpa_Model_User::addData($aUser) > 0) {
            ...
            $aParam['where']['iStatus'] = Tpa_Model_User::STATUS_TYPE_NORMAL;
            $aParam['where']['iType'] = Tpa_Model_User::TYPE_ADMIN;
            $aCustomerManager = Tpa_Model_User::getAll($aParam);
            $this->assign('aIndustry', $aIndustry);
            $this->assign('aProperty', $aProperty);
            $this->assign('aCreditLevel', $aCreditLevel);
            $this->assign('aRelationLevel', $aRelationLevel);
    ...
    public function _checkClientData($iType)
    {
        ...
        if ($iType == 2) {
            ...
            $aUser = Tpa_Model_User::getDetail($aParam['iUserID']);
            if (empty($aUser)) {
                return $this->showMsg('用户不存在！', false);
            }
        ...
        //验证邮箱
        if (!empty($aParam['sEmail'])) {
            $aUser = Tpa_Model_User::getUserByEmail($aParam['sEmail'], Tpa_Model_User::TYPE_HR);
            if (!empty($aUser['iUserID']) && $aUser['iUserID'] != $aParam['iUserID']) {
                return $this->showMsg('该邮箱已被注册！', false);
            }
        }
        //验证手机是否被注册
        if (!empty($aParam['sMobile'])) {
            $aUser = Tpa_Model_User::getUserByMobile($aParam['sMobile'], Tpa_Model_User::TYPE_HR);
            if (!empty($aUser['iUserID']) && $aUser['iUserID'] != $aParam['iUserID']) {
                return $this->showMsg('该手机已被注册！', false);
            }
        }
        ...
        //验证手机是否被注册
        if (!empty($aParam['sUserName'])) {
            $aUser = Tpa_Model_User::getUserByUserName($aParam['sUserName'], Tpa_Model_User::TYPE_HR);
            if (!empty($aUser['iUserID']) && $aUser['iUserID'] != $aParam['iUserID']) {
    ...
    public function chgpwdAction ()
    {
        if ($this->_request->isPost()) {
            ...
            $where = [
                ...
                'iType' => Tpa_Model_User::TYPE_ADMIN,
                'iStatus' => Tpa_Model_User::STATUS_TYPE_NORMAL
            ];          
            $aUser = Tpa_Model_User::getRow([
                'where' => $where
            ]);
            if ($aUser) {
                if ($aUser['sPassword'] == md5(Yaf_G::getConf('cryptkey', 'cookie') .$sOldPwd)) {
                    $where['sPassword'] = md5(Yaf_G::getConf('cryptkey', 'cookie') . $sNewPwd);
                    $result = Tpa_Model_User::updData($where);
                }               
            }
