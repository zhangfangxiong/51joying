	...
	createDialogWrap : function( tmpl ){
		...
		var header = tmpl.header ? 
			...
			noBtn = typeof tmpl.noFn === 'function' || tmpl.noFn === true ? 
				...
				'',
            // 加载按钮
            loadDiv = typeof tmpl.loadText === 'string'?
                '<div class="easyDialog_loading" id="easyDialogLoading">' + tmpl.loadText + '</div>': 'loading',
			// footer
			footer = yesBtn === '' && noBtn === '' ? '' :
				'<div class="easyDialog_footer">' + noBtn + yesBtn + loadDiv +'</div>',
...
var extend = {
	open : function(){
		...
		if(options.width != null){
			dialogWrap.style.width = options.width+'px';
		}
		if(options.height  != null){
			dialogWrap.style.height = options.height+'px';
		}
		if(options.overflow  != null){
			dialogWrap.style.overflow = options.overflow;
		}
